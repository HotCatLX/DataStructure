//
//  LXSingleCircleLinkedList.swift
//  DataStructure_Swift
//
//  Created by suckerl on 2021/3/17.
//  Copyright © 2021 suckerl. All rights reserved.
//

import Cocoa

/*
 * 单向循环链表
 */

class LXSingleCircleLinkedList<T:Equatable>: NSObject, LXCheckRangeable {
    
    var size: Int = 0
    
    override var description: String {
        var returnStr = ""
        returnStr.append("size = \(size), [")
       
        var node = first
        for i in 0 ..< size {
            if (i != 0) {
                returnStr.append(",")
            }
            returnStr.append("\(node!.val)")
            node = node?.next
        }
        returnStr.append("]")
        return returnStr
    }
    
    private class ListNode<T> {
        var val: T
        var next: ListNode?
        init(_ val: T, next: ListNode<T>?) {
            self.val = val
            self.next = next
        }
    }
    
    private var first: ListNode<T>?
    
    /*
     * 获取对应的节点
     * @param index
     * @retuen node
     * warning: 强制! 这里需要优化
     */
    private func node(index: Int) -> ListNode<T> {
        var node = first
        for _ in 0..<index {
            node = node?.next
        }
        return node!
    }
    
    /*
     * 清空所有元素
     */
    public func clear() {
        size = 0
        first = nil
    }
    
    /*
     * 返回index对应位置的节点
     * @param index
     * @retuen node
     */
    public func get(index:Int) -> T {
        let indexNode = node(index: index)
        return indexNode.val
    }
    
    /*
     * 设置index对应位置的节点
     * @param index: 位置 element: 元素
     * @retuen node
     */
    public func set(index:Int, element:T) -> T {
        let indexNode = node(index: index)
        indexNode.val = element
        return indexNode.val
    }
    
    /*
     * 添加节点到index位置
     * @param index: 位置 element: 元素
     * @retuen node
     */
    public func add(index:Int, element:T) {
        rangeCheckForAdd(index: index)
        
        if index == 0 {
            //头节点添加,first为nil或者原有头节点
            let newnode = ListNode(element,next: first)
            first = newnode
            //当只有一个节点时,first指向这个节点 & 这个节点的next指向自己
            //如果有多个节点时,first指向新的节点 & 尾节点的next指向这个节点
            let last = (size == 0) ? first : node(index: size - 1)
            last!.next = first
            
        }else {
            let prev = node(index: index - 1)
            let newnode = ListNode(element,next: prev.next)
            prev.next = newnode
        }
        size += 1
    }
    
    
    /*
     * 移除index位置对应的元素
     */
    public func remove(index: Int) -> T {
        rangeCheck(index: index)
       
        let indexnode = node(index: index)


        if index == 0 {
            if size == 1 {//唯一的节点
                first = nil
            }else {
                first = indexnode.next
                let last = node(index: size - 1)
                last.next = first
            }
            
        }else {
            let prev = node(index: index - 1)
            prev.next = indexnode.next
        }

        size -= 1
        return indexnode.val
    }
    
    /*
     * 查看元素对应的位置
     */
    public func indexOf(element:T) -> Int {
        var node = first
        guard node != nil else {
            //若无头结点,则返回0
            return 0
        }
        
        for i in 0 ..< size {
            if node?.val == element {
                return i
            }
            node = node?.next
        }
        
        return 0
    }
    
}
